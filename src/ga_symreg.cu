...
Operation genetic_sym(float *X, float *y, int sizeX, int sizey, float *cdf, int n_generations, int n_individuals, int height, int n_vars, int tournament_size, float reproduc_rate,
                      float mut_rate, float random_rate, int windowsize){
    ...
    cudaMalloc((void**)&d_cdf, NUM_OPERATORS*sizeof(float));
    ...
    cudaMemcpy(d_cdf, cdf, NUM_OPERATORS * sizeof(float), cudaMemcpyHostToDevice);
    ...
    create_population<<<numBlocks, numThreadPerBlock>>>(d_output_A, n_individuals, d_X, d_y, d_states, d_poolOP_A, d_poolTerminals_A, d_poolConsts_A, 
                                                        d_cdf, height, n_vars, n_ops, n_leaves, length, seed, sizeX, sizey);
    ...
    for (int gen = 0; gen < n_generations; gen++) {
        ...
        cub::DeviceRadixSort::SortPairs(d_temp_storage, temp_storage_bytes, d_fitness_keys, d_fitness_keys_out, d_idxIndividuals_out, d_idxIndividuals, 
                                        n_individuals);
        ...
        if(best_fitness < 1e-5f || converged) break;
    }
